...
function calcularRoseta() {
    ...
    let res, unidade, descricao;
    switch (tipo) {
        case 'corrente':
            res = valor1 / valor2; // I = P / V
            unidade = 'A';
            descricao = `Corrente calculada: ${res.toFixed(2)} A (Potência: ${valor1} W, Tensão: ${valor2} V)`;
            break;
        case 'potencia':
            res = valor1 * valor2; // P = V * I
            unidade = 'W';
            descricao = `Potência calculada: ${res.toFixed(2)} W (Tensão: ${valor1} V, Corrente: ${valor2} A)`;
            break;
        case 'secao':
            res = (2 * valor1 * valor2 * 0.0176) / (0.04 * 220); // S ≈ (2*I*L*ρ)/(ΔV*V), ρ=0.0176 Ωmm²/m, ΔV=4%, V=220V
            unidade = 'mm²';
            descricao = `Seção do cabo: ${res.toFixed(2)} mm² (Corrente: ${valor1} A, Comprimento: ${valor2} m)`;
            break;
        case 'queda':
            res = (2 * valor1 * valor2 * 0.0176 * 100) / (6 * 220); // ΔV% = (2*I*L*ρ*100)/(S*V), S=6 mm², V=220V
            unidade = '%';
            descricao = `Queda de tensão: ${res.toFixed(2)} % (Corrente: ${valor1} A, Comprimento: ${valor2} m, S=6 mm²)`;
            break;
    }

    resultado.innerHTML = `<p>${descricao}</p>`;
}
