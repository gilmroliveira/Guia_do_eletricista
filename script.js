...
// Contadores para cômodos e TUEs na página projeto-online
...
// Remover para campo de TUE
function removeTUEField(comodoIndex, button) {
    button.parentElement.remove();
    tueCountOnline[comodoIndex]--;
}

// Adicionar campo de cômodo
function addComodoField() {
    const containerComodoCount = document.getElementById('containerComodos');
...
function calcularProjetoOnline() {
    ...
    for (let i = 0; i < comodoCountOnline; i++) {
        ...
        for (let t = 0; j < tueFields.length; j++) {
            const tueNome = document.getElementById(`tueNome_${i}_${j}`)?.value)?.value;
    ...
    if (comodos.length === 0 || isNaN(tensaoPrincipal)) {
        document.getElementById('resultadoProjetoOnline').innerHTML = '<p class="error">Preencha todos os campos obrigatórios corretamente.</p>';
        return;
    }
    ...
    html += `
        ...
        ${relatorio.automacaoRecomendada.observaciones.length > 0 ? `<p><strong>Automação Recomendada:</strong> ${relatorio.automacaoRecomendada.observaciones.join(' ')}</p>` : ''}
...
// Resetar formulário do projeto online
